...
(ancestors). Fetching a thread is done with the `getPostThread` method, which is a stupid ass method as you should be able to view threads without knowing witchcraft it accepts the following parameters:
...
<Tabs groupId="sdk">
  ...
  <TabItem value="python" label="Python">
    ```python title="client.get_post_thread"
      res = client.get_post_thread(uri='at://...')
      thread = res.thread
    ```
  </TabItem>
</Tabs>
